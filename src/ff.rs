    ...
    pub fn execute(&mut self) {
        ...
        for _index in 0..self.config.population_size as usize {
            ...
            for _dim in 0..self.config.dimensions {
                temp.push(
                    thread_rng()
                        .gen_range(self.config.lower_bound as f64..self.config.upper_bound as f64),
                );
            }
                        ...
                        let const1 = self.config.beta0
                            * f64::powf(
                                ...
                                -1_f64
                                    * self.config.gamma
                                    * f64::powi(
                                        distance(&population[index], &population[innerindex]),
                                        2,
                                    ),
                            );
                        for dimension in 0_usize..self.config.dimensions as usize {
                            population[index][dimension] += const1
                                * (population[innerindex][dimension]
                                    - population[index][dimension])
                                + self.config.alfa0
                                    * alfa
                                    * (rng.gen_range(0.01..0.99)/*TODO ADD SETTING*/ - 0.5)
                                    * scale;
                        }
