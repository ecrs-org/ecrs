...
pub fn levy(x: &Vec<f64>) -> f64 {
    ...
    for x_i in x.iter().take(x.len() - 1) {
        ...
        result += f64::powi(temp - 1.0, 2)
            * (1.0 + 10.0 * f64::powi(f64::sin(f64::consts::PI * temp + 1.0), 2));
    }
    let temp: f64 = 1.0 + (x[x.len() - 1] - 1.0) / 4.0;
    result +=
        f64::powi(temp - 1.0, 2) * (1.0 + f64::powi(f64::sin(2.0 * f64::consts::PI * temp), 2));
    result
}
...
pub fn goldstein_price(x: &Vec<f64>) -> f64 {
    ...
    (1.0 + f64::powi(x1 + x2 + 1.0, 2)
        * (19.0 - 14.0 * x1 + 3.0 * f64::powi(x1, 2) - 14.0 * x2
            + 6.0 * x1 * x2
            + 3.0 * f64::powi(x2, 2)))
        * (30.0
