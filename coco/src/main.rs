...
const INDEPENDENT_RESTARTS: u64 = 1e2 as u64;
...
fn ecrs_experiment(
	...
	while let Some(problem) = &mut suite.next_problem(Some(observer)) {
		...
		if no_constraints != 0 {
			panic!("PROBLEM OF DIMENSION {} WITH {} CONSTRAINTS", dimension, no_constraints);
		}

		if no_objectives > 1 {
			panic!("PROBLEM OF DIMENSION {} WITH {} OBJECTIVES", dimension, no_objectives);
		}

		if no_int_objectives > 0 {
			panic!("PROBLEM OF DIMENSION {} WITH {} INT OBJECTIVES", dimension, no_int_objectives);
		}
